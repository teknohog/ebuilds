...
MyPV="${PV/_/}"
MyPN="primecoin-hp"
MyP="primecoin-0.1.1-hp4"

DESCRIPTION="Primecoin high performance crypto-currency wallet for automated services"
HOMEPAGE="http://primecoin.org/"
SRC_URI="mirror://sourceforge/${MyPN}/${MyP}.tar.bz2"
...
src_compile() {
	...
	cd src || die
	emake CC="$(tc-getCC)" CXX="$(tc-getCXX)" -f makefile.unix "${OPTS[@]}" primecoind
}
...
src_install() {
	...
	newconfd "${FILESDIR}/primecoin.confd" primecoind
	newinitd "${FILESDIR}/primecoin.initd" primecoind
